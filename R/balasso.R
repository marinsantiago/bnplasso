#' Fit linear regression models with a Bayesian adaptive Lasso prior
#'
#' This function fits linear regression models with a Bayesian adaptive Lasso 
#' prior as in Leng et al. (2014).
#'
...
#'   Default is \code{"conjugate"} as in Leng et al. (2014).
...
balasso.lm <- function(X, y, a, b,
  ...
  n.obs <- dimsX[1]
  n.preds <- dimsX[2] 
  n.draws <- nrow(balasso_out$Post.beta)
  balasso_out[["a"]] <- a
  balasso_out[["b"]] <- b
  balasso_out[["intercept"]] <- intercept
  balasso_out[["variance.prior.type"]] <- variance.prior.type
  balasso_out[["max.iters"]] <- max.iters
  balasso_out[["burn.in"]] <- burn.in
  balasso_out[["thin"]] <- thin
  balasso_out[["n.obs"]] <- n.obs
  balasso_out[["n.preds"]] <- n.preds
  balasso_out[["n.draws"]] <- n.draws # After burn-in and thinning 
  
  # Posterior predictive fitted values and residuals ---------------------------
  # Pre-compute all linear predictors (without the intercept)
  linPreds <- tcrossprod(X, balasso_out$Post.beta)
  # Add the intercept if needed
  if (intercept) {
    linPreds <- linPreds + matrix(
      data = balasso_out$Post.mu, nrow = nrow(linPreds),
  ...
  post_sigmas <- sqrt(abs(balasso_out$Post.sigma2) + 1e-16)
  ...
  balasso_out[["X"]] <- X
  balasso_out[["y"]] <- y
  balasso_out[["post.pred.fitted.values"]] <- post_pred_fits
  balasso_out[["post.pred.residuals"]] <- post_pred_res
  class(balasso_out) <- "lmBayes"
  balasso_out
}
