// -----------------------------------------------------------------------------
// Multivariate normal samplers, assuming an independent variance prior
// -----------------------------------------------------------------------------
...
#include <cmath>
#include "utils.h"
#include "mvn_sampler_ind_sigma.h"
 ...
 * This function assumes an independent variance prior as in                   \
 * Moran et al. (2019) <doi:10.1214/19-BA1149>.                                \
 *                                                                             \
...
Eigen::VectorXd sample_beta_ind_sigma(const Eigen::MatrixXd & X,
  ...
  Eigen::MatrixXd Phi = tXX / sigma2 + tau2_inv_mat;
  // Compute X^t * (y - mu) / sigma2
  Eigen::VectorXd y_mu = y.array() - mu;
  Eigen::VectorXd tXy_s2 = tX * y_mu / sigma2;
  // Cholesky factorization
  Eigen::LLT<Eigen::MatrixXd> llt(Phi);
  // Solve the inner and outer systems using the Cholesky factorization
  Eigen::VectorXd mu_tilde = llt.solve(tXy_s2);
  // Sample z ~ MVN(0, I_p)
 ...
 * "sample_beta_ind_sigma()", uses double precision.                           \
 *                                                                             \
 ...
 */
// [[Rcpp::export]]
Eigen::VectorXf sample_beta_ind_sigma_float(const Eigen::MatrixXf & X,
  ...
  Eigen::MatrixXf Phi = tXX / sigma2 + tau2_inv_mat;
  // Compute X^t * (y - mu) / sigma2
  Eigen::VectorXf y_mu = y.array() - mu;
  Eigen::VectorXf tXy_s2 = tX * y_mu / sigma2;
  // Cholesky factorization
  Eigen::LLT<Eigen::MatrixXf> llt(Phi);
  // Solve the inner and outer systems using the Cholesky factorization
  Eigen::VectorXf mu_tilde = llt.solve(tXy_s2);
  // Sample z ~ MVN(0, I_p)
  Rcpp::NumericVector z_Rcpp = Rcpp::rnorm(p, 0.0, 1.0);
